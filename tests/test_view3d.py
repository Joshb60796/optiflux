...
from view_3d import _pt, _w2p, build_scene


class TestWorldToPlotTransform(unittest.TestCase):
    """+90° about optical Y: X'=Z, Y'=Y, Z'=-X."""

    def test_optical_z_to_plot_x(self):
        self.assertEqual(_pt(0.0, 0.0, 10.0), (10.0, 0.0, 0.0))

    def test_optical_y_unchanged(self):
        self.assertEqual(_pt(0.0, 3.5, 0.0), (0.0, 3.5, 0.0))

    def test_optical_x_to_neg_plot_z(self):
        self.assertEqual(_pt(5.0, 0.0, 0.0), (0.0, 0.0, -5.0))

    def test_combined_point(self):
        # (x,y,z)=(1,2,3) → (3, 2, -1)
        self.assertEqual(_pt(1.0, 2.0, 3.0), (3.0, 2.0, -1.0))
    ...
    def test_w2p_arrays(self):
        ...
        np.testing.assert_allclose(py, [0.0, 2.0])
        np.testing.assert_allclose(pz, [-1.0, 0.0])

    def test_w2p_scalar(self):
        px, py, pz = _w2p(2.0, -1.0, 7.0)
        self.assertEqual((px, py, pz), (7.0, -1.0, -2.0))

    def test_light_axis_monotonic_in_plot_x(self):
        """Points along optical +Z map to increasing plot X (rightward light)."""
...
class TestBuildSceneSmoke(unittest.TestCase):
    ...
    def test_default_params_no_exception(self):
        ...
        # Optical labels after remap
        self.assertIn("Z", ax.get_xlabel())
        self.assertIn("Y", ax.get_ylabel())
        self.assertIn("X", ax.get_zlabel())
        fig.clf()
    ...
    def test_with_blockers_and_rays(self):
        ...
        # Default camera for light-to-the-right
        self.assertAlmostEqual(float(azim), -90.0, delta=1.0)
        fig.clf()
